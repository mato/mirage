...
let solo5 =
  mk_flag ["solo5"] "Generate a Solo5 unikernel. Do not use in conjunction with --unix."
let unix =
...
let mode unix xen solo5 target =
  ...
  | None -> begin
    ...
    match xen, solo5, unix with
    | false, true , true  -> failwith "Cannot specify --unix and --solo5 together."
    | true , true, false  -> failwith "Cannot specify --solo5 and --xen together."
    | true , false, true  -> failwith "Cannot specify --unix and --xen together."
    | true , false, false -> `Xen
    | false, true, false -> `Solo5
    | false, false, _ -> default_unix
  end
...
let configure =
  ...
  let configure unix xen solo5 no_macosx no_opam no_opam_version_check no_depext file =
    if unix && xen then `Help (`Pager, Some "configure")
      ...
      Mirage.set_mode (mode unix xen solo5 no_macosx);
      let t = Mirage.load file in
      `Ok (Mirage.configure t)) in
  Term.(ret (pure configure $ unix $ xen $ solo5 $ target $ no_opam $
